class PostsController < ApplicationController
	...
	def update
		@post = Post.find(params[:id])
			...
			if @post.update(params[:post].permit(:title, :profile, :description, :body, :cover, :remove_cover))
				redirect_to @post
			else
				render 'edit'
			end
	...
	private
		def post_params
			params.require(:post).permit(:title, :profile, :description, :body, :cover, :remove_cover)
		end
